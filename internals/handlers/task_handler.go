...
func GetUsersUpdateTodayHandler(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")

	userTasks, err := database.GetUsersUpdateForToday()
	if err != nil {
	...
	response, err := json.Marshal(userTasks)
	if err != nil {
		w.WriteHeader(http.StatusInternalServerError)
		_, _ = w.Write([]byte(err.Error()))
		return
...
func UpdateCommentHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err := database.UpdateComment(id, updatedComment.Content); err != nil {
		w.WriteHeader(http.StatusBadRequest)
		_, _ = w.Write([]byte(err.Error()))
	}
